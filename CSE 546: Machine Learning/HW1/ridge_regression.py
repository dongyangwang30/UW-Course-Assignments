...
def train(x: np.ndarray, y: np.ndarray, _lambda: float) -> np.ndarray:
    ...
    return np.linalg.solve(x.T.dot(x) + reg_matrix, x.T.dot(y))
...
def one_hot(y: np.ndarray, num_classes: int) -> np.ndarray:
    ...
    Example:
        ...
        ```
    """
    y_one_hot = np.zeros((y.shape[0], y.shape[0]))
...
def main():
    ...
    for i in range(x_test.shape[0]):
        if y_test[i] != y_test_pred[i]:
            right = index % 2
            left = index % 5
            ax[left,right].imshow(x_test[i].reshape((28, 28)), cmap='gray')
            ax[left,right].set_title(f"PolyRegression with true = {y_test[i]} and prediction = {y_test_pred[i]}")
            index += 1
        if index == 10:
            plt.show()
            break
