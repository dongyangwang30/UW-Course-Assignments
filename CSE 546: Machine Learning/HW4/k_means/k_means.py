...
def calculate_centers(
    ...
    for i in range(num_centers):
        centers[i] = data[np.where(classifications == i)].mean(axis = 0)
    return centers
...
def lloyd_algorithm(
    ...
    while True:
        ...
        assignments = cluster_data(data, centers)
        new_centers = calculate_centers(data, assignments, num_centers)
        error = calculate_error(data, new_centers)

        if np.max(np.abs(centers - new_centers))  <= epsilon:
            break
