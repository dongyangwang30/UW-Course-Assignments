...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    """
    part1 = vanilla_matmul(vanilla_transpose(A), x)
    part2 = vanilla_matmul(A, x) 
    part3 = vanilla_matmul(vanilla_transpose(B), y)
    res = [0] * len(part1)
    for i in range(len(part1)):
        res[i] += part1[i] + part2[i] + part3[i]
    return res
...
def numpy_solution(
    ...
    """
    return np.dot(np.transpose(A), x) + np.dot(A, x) +  np.dot(np.transpose(B),y) 
